package hogar.codelive.externalws.middleware;
...
import hogar.codelive.externalws.records.LogRule;
import hogar.codelive.externalws.constants.AppConstants;
import hogar.codelive.externalws.constants.LogConstants;
import hogar.codelive.common.middleware.MiddlewareUtil;
import hogar.codelive.externalws.records.RequestLogContext;
import hogar.codelive.externalws.constants.SecurityConstants;
...
public class HttpLoggingInterceptor implements AsyncHandlerInterceptor {
    ...
    private String extractRequestBody(HttpServletRequest request) {
        return Optional.ofNullable(WebUtils.getNativeRequest(Objects.requireNonNull(request), ContentCachingRequestWrapper.class))
    ...
    private String extractResponseBody(HttpServletResponse response) {
        return Optional.ofNullable(WebUtils.getNativeResponse(Objects.requireNonNull(response), ContentCachingResponseWrapper.class))
            .filter(wrapper -> isLoggableContentType(wrapper.getContentType()))
